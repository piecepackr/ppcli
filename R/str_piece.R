...
str_piece_helper <- function(
	...
	df <- clean_df(df)
	if (isTRUE(reorient) || reorient == "all") {
		df$angle <- 0
	}
	...
	cm <- list(
		char = matrix(style$space, nrow = nr, ncol = nc),
...
color_text <- function(cm, color) {
	if (color == "html") {
		# always colorize if we'll be converting to html
...
clean_df <- function(df) {
	...
	df$rank <- ifelse(
		df$rank == 1L & str_detect(df$piece_side, "^board") & str_detect(df$cfg, "[12]$"),
		8L,
		df$rank
	)
	# go board rank is number of lines
	df$rank <- ifelse(
		str_detect(df$piece_side, "^board") & df$cfg == "go",
		ifelse(df$rank == 1L, 18L, df$rank - 1),
		df$rank
	)
	# marbles board rank is number of holes
	df$rank <- ifelse(
		str_detect(df$piece_side, "^board") & df$cfg == "marbles",
		ifelse(df$rank == 1L, 4L, df$rank),
		df$rank
	)
	# alquerque board always has four "cells"
	df$rank <- ifelse(str_detect(df$piece_side, "^board") & df$cfg == "alquerque", 4L, df$rank)
	# morris rank is number of men
	df$rank <- ifelse(
		str_detect(df$piece_side, "^board") & df$cfg == "morris",
...
get_df_offsets <- function(df, lr, xoffset, yoffset, annotate = FALSE) {
	...
	if (is.null(xoffset)) {
		xoffset <- min2offset(lr$xmin, xlbound)
	}
	if (is.null(yoffset)) {
		yoffset <- min2offset(lr$ymin, ylbound)
	}
...
add_piece <- function(
	...
	if (
		...
		if (grepl("chess", cfg) && suit == 6L) {
			rs <- unicode_chess_white[rank]
		}
		if (grepl("reversi", cfg) && suit == 6L) {
			rs <- "\u26c1"
		}
		if (grepl("checkers", cfg) && suit == 6L) {
			rs <- "\u26c1"
		}
...
add_matchstick_face <- function(cm, x, y, angle, fg, rank) {
	switch(
		...
		abort(paste("Don't know how to draw matchstick of rank", rank), class = "unicode_diagram")
	)
}
abort_angle <- function(angle) {
	abort(paste("Can't handle angle", angle), class = "unicode_diagram", angle = angle)
}
...
get_dots <- function(rank) {
	switch(
		...
		"\u20e8\u0308",
		"\u20e8\u20db",
		abort(paste("Doesn't support", rank, "dots")),
...
add_tile_back_subpack <- function(cm, x, y, style) {
	...
	cm <- add_gridlines(cm, x, y, 1, 1, 0.5, has_pua_box_drawing = style$has_pua_box_drawing)
	cm
}
...
add_morris_board <- function(
	...
	hv <- 1L # light
	if (rank == 2L) {
		# three men's morris without diagonals
		cm <- add_border(cm, x, y, width, height, space = style$space)
