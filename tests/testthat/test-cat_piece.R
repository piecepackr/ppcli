...
test_that("text diagrams", {
	...
	expect_error(suppressWarnings(cat_piece(tibble(
	...
	expect_snapshot({
		...
		df <- dplyr::mutate(df, cfg = gsub("checkers1", "checkers2", cfg), x = 2 * x, y = 2 * y)
	...
	expect_snapshot({
		...
		dfpt <- tibble(
			...
			x = 1:8,
			y = 4,
			rank = rep(1:3, length.out = 8),
			suit = c(1:6, 1:2),
			angle = seq(0, by = 45, length.out = 8),
			cfg = "icehouse_pieces"
		)
		dfpf <- tibble(
			piece_side = rep(c("pyramid_face", "pyramid_left", "pyramid_right", "pyramid_back"), 6),
			x = rep(1:8, 3),
			y = rep(1:3, each = 8),
			rank = rep(1:3, each = 8),
			suit = rep(1:6, 4),
	...
	expect_snapshot({
		...
		dfbt <- tibble(
			piece_side = rep(c("tile_face", "tile_back"), 2),
			x = 1:4,
			y = 1,
			suit = 1:4,
			rank = 1:4,
		...
		dfc <- tibble(
			piece_side = rep(c("coin_face", "coin_back"), 2),
			x = 1:4,
			y = 2,
			suit = 1:4,
			rank = 1:4,
		...
		dfp <- tibble(
			piece_side = rep(c("pawn_face", "pawn_back"), 2),
			x = 1:4,
			y = 4,
			suit = 1:4,
			rank = 1:4,
	...
	expect_snapshot({
		...
		dfpb <- tibble(piece_side = "pawn_back", x = 1:2, y = 1, suit = 2:1, angle = c(0, 45))
	...
	expect_snapshot({
		...
		df3 <- tibble(piece_side = "board_face", x = 2, y = 2, rank = 3L, suit = 3L, cfg = "morris")
		dfs <- tibble(
			piece_side = "bit_back",
			x = rep(1:3, 2),
			y = rep(1:2, each = 3),
		...
		df6 <- tibble(piece_side = "board_face", x = 3, y = 3, rank = 6L, suit = 3L, cfg = "morris")
		cat_piece(df6)

		df7 <- tibble(piece_side = "board_face", x = 3, y = 3, rank = 7L, suit = 3L, cfg = "morris")
		cat_piece(df7)

		df9 <- tibble(piece_side = "board_face", x = 4, y = 4, rank = 9L, suit = 3L, cfg = "morris")
		cat_piece(df9)
